...
func setupDB(t *testing.T) (*sql.DB, func()) {
	...
	createTables := []string{
		...
		`CREATE TABLE IF NOT EXISTS users (user_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), username VARCHAR(255) UNIQUE NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
		`CREATE TABLE IF NOT EXISTS likes (id SERIAL PRIMARY KEY, actor_user_id UUID NOT NULL, recipient_user_id UUID NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(actor_user_id, recipient_user_id), FOREIGN KEY (actor_user_id) REFERENCES users(user_id), FOREIGN KEY (recipient_user_id) REFERENCES users(user_id));`,
		`CREATE TABLE IF NOT EXISTS decisions ( actor_user_id UUID NOT NULL, recipient_user_id UUID NOT NULL, liked_recipient BOOLEAN NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(actor_user_id, recipient_user_id), FOREIGN KEY (actor_user_id) REFERENCES users(user_id), FOREIGN KEY (recipient_user_id) REFERENCES users(user_id));`,
	}
...
func TestIntegrationGetLikedYou(t *testing.T) {
	...
	likers, err := repo.GetLikedYou(ctx, recipientUserID.String(), limit, offset)
...
func TestIntegrationGetNewLikedYou(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	likers, err := repo.GetNewLikedYou(ctx, recipientUserID.String(), limit, offset)
	require.NoError(t, err)
...
func TestIntegrationCountLikes(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	count, err := repo.CountLikes(recipientUserID.String())
	require.NoError(t, err)
...
func TestIntegrationInsertDecision(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	err := repo.InsertDecision(actorUserID.String(), recipientUserID.String(), likedRecipient)
	require.NoError(t, err)

	// Verify insertion
	var liked bool
	query := "SELECT liked_recipient FROM decisions WHERE actor_user_id = $1 AND recipient_user_id = $2"
	err = db.QueryRow(query, actorUserID, recipientUserID).Scan(&liked)
	require.NoError(t, err)
...
func TestIntegrationInsertLike(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	err := repo.InsertLike(actorUserID.String(), recipientUserID.String())
	require.NoError(t, err)

	// Verify insertion
	var exists bool
	query := "SELECT EXISTS (SELECT 1 FROM likes WHERE actor_user_id = $1 AND recipient_user_id = $2)"
	err = db.QueryRow(query, actorUserID, recipientUserID).Scan(&exists)
	require.NoError(t, err)
...
func TestIntegrationDeleteLike(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	err := repo.DeleteLike(user1ID.String(), recipientUserID.String())
	require.NoError(t, err)

	// Verify deletion
	var exists bool
	query := "SELECT EXISTS (SELECT 1 FROM likes WHERE actor_user_id = $1 AND recipient_user_id = $2)"
	err = db.QueryRow(query, user1ID, recipientUserID).Scan(&exists)
	require.NoError(t, err)
...
func TestIntegrationCheckMutualLike(t *testing.T) {
	...
	repo := repository.NewExploreRepository(db)
	exists, err := repo.CheckMutualLike(recipientUserID.String(), user1ID.String())
	require.NoError(t, err)
	assert.True(t, exists)
}
